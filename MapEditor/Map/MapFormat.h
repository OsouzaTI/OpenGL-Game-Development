...
typedef struct
{//TODO: estudar essa estrutura, apesar de pequena... é a mais complexa
	...
	if (header.max_objects <= 0) {
		...
		for (long i = 0; i < header.max_objects; i++)
		{
			...
			if (temp[i].max_vertices > 0) {
				//TODO: estudar a diferença de copiar o conteudo ao inves dos dados
...
{
	// Naõ será permitido cores com intensidade total
	if (
		(r == 255	&& g == 0	&& b == 0)	 ||
		(r == 0		&& g == 255 && b == 0)	 ||
		(r == 0		&& g == 0	&& b == 255) ||
